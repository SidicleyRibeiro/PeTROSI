...
class MpfaD2D:
    ...
    def face_weight(self, interp_node, face):
        coords_interp_node = self.mb.get_coords([interp_node])
        ...
        for adjacent_volume in adjacent_volumes:
            ...
            cot_num = aux_dot_num / (2.0 * self.trian_area(half_face, half_other_face, coords_interp_node))
            # print("cot_num: ", cot_num, coords_interp_node, half_face, self.mesh_data.get_centroid(adjacent_volume))
            vector_pseudo_face = half_face - half_other_face
            normal_pseudo_face = self.area_vector(half_face, half_other_face, coords_interp_node)
            ...
            # print("")
            K_den_n = self._get_conormal_prod(np.asarray([coords_interp_node, half_face]), centroid_volume, perm_volume)

            aux_dot_den = np.dot(half_face - coords_interp_node, centroid_volume - coords_interp_node)
            cot_den = aux_dot_den / (2.0 * self.trian_area(half_face, centroid_volume, coords_interp_node))

            K_den_t = self.K_t_X(np.asarray([coords_interp_node, half_face]), centroid_volume, perm_volume)
    ...
    def partial_weight(self, interp_node, adjacent_volume):
        coords_interp_node = self.mb.get_coords([interp_node])
        ...
        adjacent_faces = list(set(self.mtu.get_bridge_adjacencies(interp_node, 0, 1)) &
        ...
        K_ni_first = self._get_conormal_prod(np.asarray([coords_interp_node, half_first_face]),
                           cent_adj_vol, perm_adjacent_volume)
        K_ni_second = self._get_conormal_prod(np.asarray([coords_interp_node, half_second_face]),
                            cent_adj_vol, perm_adjacent_volume)
        ...
        # print("netas: ", neta_first, neta_second, h_first, h_second, coords_interp_node, cent_adj_vol)
        csi_first = self.face_weight(interp_node, first_face)
        csi_second = self.face_weight(interp_node, second_face)

        node_weight = K_ni_first * neta_first * csi_first + K_ni_second * neta_second * csi_second
        # print("weight: ", node_weight, coords_interp_node, cent_adj_vol)
        return node_weight
